...
def run_predefined_tests2():
    ...
    for idx, (n, min_c, max_c, density) in enumerate(tests10, 1):
        print(f"\n--- Test {idx}: {n} vertices, {density} density ---")
        graph, source, sink = generate_random_flow_network(n, min_c, max_c, density)
        network = FlowNetwork(graph, source, sink)
        labels = get_node_labels(n)
    ...
    for idx, (n, min_c, max_c, density) in enumerate(tests100, 1):
        print(f"\n--- Test {idx}: {n} vertices, {density} density ---")
        graph, source, sink = generate_random_flow_network(n, min_c, max_c, density)
        network = FlowNetwork(graph, source, sink)
        labels = get_node_labels(n)
    ...
    for idx, (n, min_c, max_c, density) in enumerate(tests1000, 1):
        print(f"\n--- Test {idx}: {n} vertices, {density} density ---")
        graph, source, sink = generate_random_flow_network(n, min_c, max_c, density)
        network = FlowNetwork(graph, source, sink)
        labels = get_node_labels(n)
...
def main():
    ...
    while True:
        print("\nOptions:")
        print("1. Run 3 predefined tests")
        print("2. Run scaling predefined tests for time analysis")
